...
tandem_copy_list <- read.csv("pan_matrix_numeric.txt",sep = "\t",header=FALSE)
...
base_pan_phylo <- read.csv("~/Desktop/NAM_PAN_GENOME/pan_genome_nov 2/phylo/phylo_info.txt",sep = "\t",header =FALSE)
names(base_pan_phylo) <- c("Query_gene","Evi_ab","Phylo")

# add in canoical information 

pan_gene_canonical <- read.csv("pan_gene_canonical.csv")
...
stack_all <- sum_total %>% 
  ...
  theme_classic()+ ylab("Propotion Phylostrata") + 
