...
#posterior simulated
post_sfs <- read_delim(
  "NAM_DFE_ABC/post_sfs.txt", delim = " ",
  col_names = FALSE
) %>%
  dplyr::select(-ncol(.)) %>% 
...
emprical_sfs <- read_delim(
  "NAM_DFE_ABC/NAM_SFS-dup_inv_tra_ins_del_all.txt", delim = "\t",
  col_names = FALSE
) %>%
  dplyr::select(-ncol(.)) %>% 
  dplyr::select(all_of(to_keep)) #%>% 
...
c + ggtitle("A") + xlab("allele frequency") + b + ggtitle("B") + xlab("allele frequency") + plot_layout(ncol = 2, widths = c(0.2, 0.8)) + 
  ggsave(filename = "NAM_DFE_ABC/figures/postcheck_viz.pdf", width = 15, height = 10)
...
posts_df %>% 
  group_by(type, idx, mu_sv) %>% 
  summarise(
    ssq = sum((empirical - simulated)^2) / sum(empirical),
    prop_gt = mean(empirical > simulated)) %>% 
  ggplot(aes(idx, ssq)) +
  geom_point() +
  facet_wrap(~type, scales = "free") 
  #facet_wrap(~type + mu_sv, scales = "free") 
...
posts_df %>% 
  ggplot() +
  geom_line(aes(idx, empirical, group = window), alpha = 1) +
  geom_point(aes(idx, simulated, group = window, colour = draw)) +
  #facet_wrap(~type + mu_sv, scales = "free") +
  facet_wrap(~type, scales = "free") +
  theme(legend.position = "n") +
  ggsave("NAM_DFE_ABC/figures/postcheck_sfs.png")
